    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div className="page-header">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
          ...
          <button 
            className={`btn ${aModule?.status === 'enabled' ? 'btn-secondary' : 'btn-primary'}`}
          ...
          >
            <span style={{
              ...
              height: '8px',
              borderRadius: '50%',
              backgroundColor: aModule?.status === 'enabled' ? 'var(--color-success)' : 'rgba(255,255,255,0.4)',
              display: 'inline-block'
            }} />
            {aModule?.status === 'enabled' ? 'Module Enabled' : 'Module Disabled'}
          </button>
        </div>
      </div>
