...
import { Shield, CheckCircle2, AlertTriangle, Play, Check, ChevronRight, RefreshCw, Loader } from 'lucide-react';
...
export function RestoreWizard({ isOpen, onClose, onSuccess }: RestoreWizardProps) {
  const [step, setStep] = useState(1);
  const [selectedPoint, setSelectedPoint] = useState<string>('rp-1');
  ...
  useEffect(() => {
    if (step === 3 && safetyCheck === 'pending') {
      ...
      const timer1 = setTimeout(() => {
        setSafetyLogs(prev => [...prev, '✓ Checking Discord Gateway connectivity... Status: EXCELLENT (14ms latency)']);
      }, 600);
      ...
      const timer4 = setTimeout(() => {
        setSafetyLogs(prev => [...prev, '✓ Verifying backup encryption signature... AES-256 Verified']);
      ...
      const interval = setInterval(() => {
        setRestoreProgress(prev => {
          ...
          if (next >= 100) {
            ...
            setTimeout(() => {
              setStep(5);
              onSuccess('Server assets successfully restored to backup point.');
            }, 500);
            return 100;
          }

          if (next < 25) {
            setRestoreStatus('Injecting guild schema structures...');
          } else if (next < 50) {
            setRestoreStatus('Rebuilding category layouts and permissions...');
          } else if (next < 75) {
            setRestoreStatus('Configuring roles hierarchy and bot hierarchy overrides...');
          } else {
            setRestoreStatus('Finalizing expressions mapping and verifying integrity...');
          }
      ...
      <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '550px' }}>
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <h3 style={{ fontSize: '16px', fontWeight: 600 }}>Server Restore Wizard</h3>
          </div>
          ...
          {step === 1 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>
                Select the backup recovery point you would like to restore the server state to:
              </div>
              
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {[
                  { id: 'rp-1', label: 'Manual Restore checkpoint (Pre-Staff Audit)', date: 'Today, 11:24 AM', size: '142.4 MB', type: 'Differential' },
                  { id: 'rp-2', label: 'Scheduled Daily Sync (AutoBackup)', date: 'Yesterday, 3:00 AM', size: '254.1 MB', type: 'Full Backup' },
                  { id: 'rp-3', label: 'System emergency snapshot (Anti-Nuke Trigger)', date: 'June 23, 8:41 PM', size: '251.9 MB', type: 'Full Backup' }
                ].map(point => (
                  <div
                    key={point.id}
                  ...
                  >
                    <div>
                      <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)' }}>{point.label}</div>
                      <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{point.date} • {point.type}</div>
                    </div>
                    <div style={{ fontSize: '11px', color: 'var(--text-secondary)', fontWeight: 600 }}>{point.size}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {[
                  ...
                  { key: 'settings', title: 'Guild Server Settings', desc: 'Verification levels, system channels, onboarding config' },
                  { key: 'webhooks', title: 'Webhooks & Integrations', desc: 'Restores authorized secure system webhooks' },
                  { key: 'expressions', title: 'Guild Expressions', desc: 'Restores custom emojis, stickers, and soundboard sounds' }
                ].map(item => (
                  <label
                    key={item.key}
                    style={{
