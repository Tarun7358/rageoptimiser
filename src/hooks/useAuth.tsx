...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [elevatedToken, setElevatedTokenState] = useState<string | null>(localStorage.getItem('cn_elevated_token'));
  const [elevationCallback, setElevationCallback] = useState<(() => void) | null>(null);
  ...
  const setElevatedToken = (newToken: string | null) => {
    setElevatedTokenState(newToken);
    if (newToken) localStorage.setItem('cn_elevated_token', newToken);
    else localStorage.removeItem('cn_elevated_token');
  };

  const requireElevation = (callback: () => void) => {
    if (elevatedToken) callback();
    else setElevationCallback(() => callback);
  };

  const cancelElevation = () => setElevationCallback(null);
  ...
  return (
    <AuthContext.Provider value={{
      ...
      elevatedToken, setElevatedToken, requireElevation, elevationCallback, cancelElevation,
